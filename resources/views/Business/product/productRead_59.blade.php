    ...
    <div class="productRead" >
        <form class="layui-form" id="product_add">
            ...
            @if($cat_id != 41 && !empty($list[0]))
                <div class="layui-form-item">
                    <label class="layui-form-label">芝麻积分 :</label>
                    <div class="layui-input-block">
                        <select name="Jifen" required>
                            <?php $title = empty($list[0]) ? [] : $list[0]['jifen'];?>
                            @foreach($title as $v)
                                <option value="{{ $v }}" @if($v === $product['Jifen']) selected="" @endif>{{ $v }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
