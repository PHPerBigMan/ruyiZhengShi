        ...
        <form class="layui-form" id="product_add">
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <input type="text" name="pNumber"  autocomplete="off" class="layui-input" value="<?php echo empty($product['pNumber']) ? $pNumber : $product['pNumber'];?>" placeholder="随机产品编号">
                </div>
            </div>
            <div class="layui-form-item">
                <label class="layui-form-label">利息 :</label>
                <div class="layui-input-block">
                    <input type="number" name="accrual"  autocomplete="off" class="layui-input" value="{{ $product['accrual'] }}" placeholder="请填写数字">
                </div>
            </div>
            <div class="layui-form-item">
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <input type="text" name="audit_time"  autocomplete="off" class="layui-input" value="{{ $product['audit_time'] }}" placeholder="单位（天）">
                </div>
            </div>
            <div class="layui-form-item">
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <select name="property">
                            <?php $title = empty($list[0]) ? ['测试','测试'] : $list[0]->property;?>
                            @foreach($title as $v)
                                <option value="{{ $v }}" @if($v === $product['property']) selected="" @endif>{{ $v }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <select name="is_mortgage">
                        <?php $title = ['无抵押','不限'];?>
                        @foreach($title as $v)
                            <option value="{{ $v }}" @if($v === $product['is_mortgage']) selected="" @endif>{{ $v }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            @if($type === '1')
               ...
               <span class="layui-btn layui-btn-normal read-btn" onclick="save()" style="margin: 20px 0 5px 400px!important">添加</span>
                @elseif($type === '2')
                <input type="hidden" name="id" value="{{ $id }}">
                <span class="layui-btn layui-btn-normal read-btn" onclick="save()" style="margin: 20px 0 5px 400px!important">修改</span>
                @endif
        </form>
    </div>
@endsection
